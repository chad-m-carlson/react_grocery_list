...
//DESTRUCTURE THE PROPS, REMOVE ALL TRACES OF props. FROM HTML {}
  ...
  const List = ({groceries, name, groceryClick, deleteGrocery}) => (
    <div>
      <h2>{name}</h2>
      <ul className="ui list">
        { groceries.map( grocery => <Grocery key={grocery.id}{...grocery} groceryClick={groceryClick} deleteGrocery={deleteGrocery}/>)}
